...
public class DataBaseOp extends SQLiteOpenHelper
{
  ...
  // CreTe Creafe Query.
  {
    try {
      ...
      dbs.execSQL(query);
      query = "CREATE TABLE IF NOT EXISTS fourth_year(Subject text, Mode text, AbsentRolls text, Date text, StartRoll int, EndRoll int);";
      dbs.execSQL(query);
      ...
      dbs.execSQL(query);
      query = "CREATE TABLE IF NOT EXISTS fourth_year_contacts(Roll text, Name text, Mobile text);";
      dbs.execSQL(query);
...
//                        }
  ...
  String getYear(String table)
  {
    ...
    if(table.contains("4th_Year"))
      return "fourth_year_contacts";
    else
      return null;
  }
